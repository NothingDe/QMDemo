...
bool MWidgetBase::nativeEvent(const QByteArray& eventType, void* message, qintptr* result)
#else
 ...
 */
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
bool MWidgetBase::winMouseEvent(MSG* msg, qintptr* result)
#else
...
#endif
{
    ...
    // 实现标题栏功能
    if (m_titleBar && m_titleBar->rect().contains(pos)   // 如果鼠标在标题栏中移动则进行处理
